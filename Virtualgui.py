...
class Widget():
    ...
    # ------------------------------------------------------KEY WORD LISTS---------------------------------------------------------------------------
    #                THESE ARE ALL THE LIST WHICH CONTAINS IMPORTANT KEYWORDS WHICH THE Damon CAN USE.
    # --------------------------------------------------------------------------------------------------------------------------------------------------------
    greet = ['Hey Whats up! How are you doing', 'Hello Kavya',
             'Hi ! I am your Assistant', 'Hello, How can I help You']
    how = ['I am Fine, What about You', 'I am fine as always',
           'what you think', 'just fine!!']
    name = ['You Can call me Damon', 'My Name Is Damon',
            'you named me Damon', 'You Should Know this, My name Is Damon']
    creator = ['You made Me', 'I was Made by Kavya',
               'One And only, Kavya', 'Best in the World, Kavya']
    can = ['I Can Do Everything .', 'Just Give Me A Try And Figure This Out.',
           'What you Coded Within Me ;) ', 'Your Choice']
    c_un = ["I did'nt get that", 'What You Said?',
    ...
    me = ['You Told Me Your Name, Kavya',
          'I Think That\'s Kavya', 'Your Name That I Know Is Kavya']
    thanks = ['My Pleasure', 'Welcome',
              'Ohh Don\'t embarrass me by saying thanks ', 'So Sweet!!']
    # ----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    # ---------------------------PRINTING FUNC---------------------------------------------------
    #       THIS FUCTION PRINTS THE Damon'S RESPONCE TO SCREEN.
    # ---------------------------------------------------------------------------------------------------
    ...
    # -----------------SPEAK FUNCTION-------------------------------
    #    THIS FUNCTION SPEAKS THE Damon'S RESPONCE
    # ------------------------------------------------------------------------
    ...
    # -----------------------SEND FUNC-----------------------------------------------------------------
    #        THIS FUCTION IS BEING USED TO SEND USER_INPUT TO Damon.
    # --------------------------------------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------------------------------------
        ...
        # ------------------------------------CONDITIONS------------------------------------------------------------------------------------------
        # THESE ARE CONDITIONS WHICH THE Damon CHECKS. HE RESPOND TO THE MOST RELEVENT CONDITION.
        # -------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        # -----------------GURPARTAP CONDITION-------------------------------------------------------------------
        elif 'Damon' in user_input:
            out = 'Damon Is A Good Friend Of Kavya. He is well Known as Damon'  # |
            self.printing_func(out)  # |
            self.speak(out)
        # -----------------------------------------------------------------------------------------------------------------------
       ...
       # --------------------------THANKS CONDITION-------------------------------------------------------
        ...
        # -----------------------------GOOD MORNING CONDITION--------------------------------------------------------------------------------
            ...
            out = f'Good Morning Kavya, The Current Time is {time},  And {outa},    Have A Good Day.'
            self.printing_func(out)
            self.speak(out)
        # --------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        # ----------------OPEN CONDITIONS -----------------------------------------
            ...
            if 'google' in user_input:
                ...
                self.printing_func(out)
                self.speak(out)
                path = r"C:\Users\Hardeep\AppData\Local\Programs\Microsoft VS Code\Code.exe"
                os.startfile(path)
        ...
        # --------------------WIKIPEDIA CONDITION-----------------------------
            ...
            try:  # |
                out = 'According To Wikipedia ' + \
                    wikipedia.summary(to2, 2)  # |
                self.printing_func(out)  # |
                self.speak(out)  # |
            except:  # |
                out = 'cannot find'  # |
        ...
        # -------------------ELSE CONDITION----------------------------------------
            ...
            res = mb.askquestion(
                'Google Search', 'May I Search That On Google.')
            ...
            if res == 'yes':
                ...
                self.speak(out)
                webbrowser.open(
                    'https://www.google.co.in/search?q=' + to_search)
            else:
                out = 'Ok!!'
                self.printing_func(out)
                self.speak(out)
    ...
    def __init__(self):
        ...
        Label(self.win, text='ASSISTANT', font=('arial black', 18),
              fg='white', width=30, bg='green', bd=5).pack()

        Label(self.win, text='Me', font=('arial black', 20),
              fg='white', bg='orange').place(x=60, y=50)
        Label(self.win, text='Damon', font=('arial black', 20),
              fg='white', bg='orange').place(x=260, y=50)

        self.text_box1 = Text(self.win, font=(
            'arial black', 13), width=16, height=5, fg='blue', wrap=WORD)
        self.text_box1.place(x=10, y=100)

        self.text_box2 = Text(self.win, font=(
            'arial black', 13), width=15, height=5, fg='orange', wrap=WORD)
        self.text_box2.place(x=200, y=100)

        self.search_var = StringVar()

        Entry(self.win, font=('arial black', 14), width=18,
              textvariable=self.search_var, bd=5).place(x=10, y=250)

        send = Button(self.win, text='Send', font=('arial black', 10), bg='blue',
                      fg='white', bd=5, width=10, command=self.send_func).place(x=270, y=250)
